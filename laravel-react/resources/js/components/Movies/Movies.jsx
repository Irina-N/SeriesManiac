...
import { getTopMovies, loadMoreMovies, searchMovies } from '../../store/actions/movies';
...
export const Movies = () => {
  ...
  const loadMore = () => {
    dispatch(loadMoreMovies({counter: paginateCounter + 1, query: searchQuery}));
  ...
  const handlerSearch = (e) => {
    ...
    if (searchText.trim()?.length > MINIMUM_QUERY_LENGTH_FOR_SEARCH
        ...
        <div className="container">
          <div id="film-container" className="row align-items-center">
            {(preloader || !topMovies || !topMovies.length)
              ? <div className="d-flex justify-content-center align-items-center h-100" ><Spinner animation="border" variant="dark" /></div>
              : topMovies.map((movie) => {
