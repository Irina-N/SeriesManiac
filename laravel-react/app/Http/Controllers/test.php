...
class test extends Controller
{
    ...
    {   
        ...
        $movies = Grade::where('user_id',10)->get();//<--Мои фильмы
                ...
                if(!in_array($users[$b]['user_id'], $usersWithIntersection)){
                    ...
                    for($c = 0; $c < count($userFilms); $c++){
                        if(in_array($userFilms[$c]['movies_id'], $filmsId)){
                            $currentUserGrade = Grade::where('user_id',10)->where('movies_id',$userFilms[$c]['movies_id'])->first();//<--Моя Оценка этого фильма
                            $commonFilmsSum ++;
